...
const CurrentApplicationPage = () => {
  ...
  const sendApplication = async () => {
    try {
      const response = await axios(`http://localhost:8000/requests/send`, {
        method: 'PUT',
  ...
  const deleteApplication = async () => {
    try {
      const response = await axios(`http://localhost:8000/requests/delete`, {
      method: 'DELETE',
      withCredentials: true
    })
      ...
      <div className={styles['application__page-wrapper']}>
        ...
        {products.length !== 0 ? <div>
          ...
          <div className={styles['application__page-info']}>
            ...
            <h3 className={styles['application__page-info-title']}>Добавленные категории:</h3>
